...
fn over(session: &Arc<Session>) -> Runner {
    Runner::new(
...
fn resuming(
    ...
    run(said, &mut renderer, runner, &mut held, &terms(sample))
...
fn a_directory_nothing_was_recorded_in_says_so() {
    ...
    let mut runner = over(&session);

    let (written, _) = resuming("", &sample, &mut runner, session);
...
fn an_id_that_names_nothing_says_so_and_shows_the_list_again() {
    ...
    for said in ["the second one", absent.as_str()] {
        let (written, _) = resuming(said, &sample, &mut runner, Arc::clone(&session));
...
fn picking_one_up_makes_it_the_session_being_recorded_to() {
    ...
    let session = Arc::new(Session::nowhere());
    let mut runner = over(&session);
    let (written, now) = resuming(&id, &sample, &mut runner, session);
...
fn the_session_already_open_is_refused_as_the_one_being_used() {
    ...
    let (written, now) = resuming(&id, &sample, &mut runner, open);
...
fn what_was_being_recorded_to_is_closed_and_stays_readable() {
    ...
    let (written, now) = resuming(&id, &sample, &mut runner, leaving);
...
fn the_transcript_a_session_replaces_is_not_left_standing_above_it() {
    ...
    run(
        ...
        &mut renderer,
        &mut runner,
        &mut lent(&mut input, &opening, session),
        &terms(&sample),
    )
    .expect("the terminal to be written");
...
fn the_preview_holds_the_work_a_session_did_and_not_only_what_was_said() {
    ...
    let runner = over(&Arc::new(recorded(&sample, "another session entirely")));
