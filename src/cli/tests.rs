...
fn the_flag_names_the_model_over_anything_a_file_says() {
    ...
    assert_eq!(
        &*wanted(&choice("claude-opus-5"), &settings),
        "claude-opus-5"
    );
}
...
fn a_provider_with_no_model_after_it_takes_the_one_configured_for_it() {
    ...
    assert_eq!(&*wanted(&choice("openai/"), &settings), "gpt-5.2");

    // And the default provider, which is what an absent flag resolves to.
    assert_eq!(&*wanted(&choice(""), &settings), "claude-opus-5");
}

#[test]
fn a_model_named_nowhere_at_all_is_the_one_this_is_built_with() {
    assert_eq!(&*wanted(&choice(""), &Settings::default()), MODEL);

    // Configured for another provider is not configured for this one.
    let sample = Sample::new("model-elsewhere");
    let elsewhere = sample.settings(r#"{"providers": {"openai": {"model": "gpt-5.2"}}}"#);

    assert_eq!(&*wanted(&choice(""), &elsewhere), MODEL);
}
