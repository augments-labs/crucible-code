...
// the one web source a run builds, and account renewals and login requests,
// each counting work it gave up on that is still running. An owner added to
// the blocking threads is added here.
const _: () = assert!(
    ToolWorker::CAPACITY + WEB_IN_FLIGHT + Renewals::BLOCKING < BLOCKING,
    "the tool worker, the web source and account requests together would take every blocking \
     thread the runtime has"
);
