...
#[test]
fn serving_again_reads_the_environment_it_was_handed_and_no_other() {
    // The lookup is the caller's, so that whoever sets a provider up again
    // mid-run says what environment there is. A closure that went to the
    // process for it would make every test of a switch depend on what the
    // person running the tests has exported, and would put their key into a
    // provider built by a fixture. Both halves: a variable only the handed
    // lookup holds is found, and one only the process could hold is not.
    let sample = Sample::new("serving-again-lookup");
    let settings =
        sample.user(r#"{"providers": {"openai": {"apiKeyEnv": "CRUCIBLE_FIXTURE_ONLY_KEY"}}}"#);
    let stored = StoredCredentials::default();
    ...
    for one in every() {
        assert!(
            matches!(
                empty(one, &stored),
                Err(AppError::Authentication { provider }) if &*provider == one.name
            ),
            "{} was set up from somewhere other than the lookup",
            one.name
        );
    }
}
