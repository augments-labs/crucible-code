...
///
/// Blocking work is disk and platform calls, the web source's requests and
/// account renewals' lock, file and lookup work, which have no asynchronous
/// form, each bounded by its owner. What every owner may hold at once is
/// checked against this in [`crate::services`]: the tool worker's four jobs,
/// the web source's two requests and account work's two threads, each counting
/// work it gave up on that is still running, and one thread to spare, so an
/// owner at its most never makes another's job queue.
pub const BLOCKING: usize = 9;
