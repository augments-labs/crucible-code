...
use crate::EXTENSION_MANIFEST_BYTES;

/// A tree of real files that exists while the test does.
///
/// What a sweep answers is decided by directory entries, file lengths and the
/// order names sort in, so a fake filesystem here would only be testing the
/// fake's answers to those questions.
...
impl Scratch {
    ...
    fn new(name: &str) -> Self {
        let path =
            std::env::temp_dir().join(format!("crucible-extensions-{name}-{}", std::process::id()));
    ...
    /// Creates a directory inside it.
    fn make(&self, at: &str) {
        fs::create_dir_all(self.at(at)).expect("a writable temporary directory");
    }

    /// Writes a file inside it, creating the directories above it.
    fn write(&self, at: &str, text: &str) {
        let path = self.at(at);
...
fn a_machine_with_nothing_installed_answers_empty_rather_than_refusing() {
    ...
    let found = Extensions::discover(&scratch.at("home"));
...
fn a_manifest_over_the_boundary_is_refused_by_its_length_rather_than_its_syntax() {
    ...
    let padding = " ".repeat(EXTENSION_MANIFEST_BYTES);
    scratch.write(
    ...
    assert!(
        said.contains(&EXTENSION_MANIFEST_BYTES.to_string()),
        "{said}"
    );
}
