...
struct CompatibilityLaunch {
    ...
    invocation: crucible_sandbox::SandboxInvocationMode,
...
mod tests {
    ...
    fn startup_stage_cleanup_failure_is_audited_as_failed() -> std::io::Result<()> {
        use crate::process::{Stage, testing_plan};
        ...
        let plan = testing_plan(
            crucible_sandbox::SandboxSpeech::Closed,
        ...
        assert!(
            facts.iter().any(|record| matches!(
                record.fact().kind(),
                crucible_sandbox::SandboxFactKind::Cleanup(
        ...
        assert!(!facts.iter().any(|record| matches!(
            record.fact().kind(),
            crucible_sandbox::SandboxFactKind::Cleanup(crucible_sandbox::SandboxCleanup::Complete)
        )));
        assert!(matches!(
            result,
            Err(crucible_sandbox::SandboxError::Lifecycle(_))
        ));
        assert!(root.exists());
        Ok(())
    }
    ...
    use crucible_sandbox::{
        SandboxAudit, SandboxCommandPolicy, SandboxCommandRule, SandboxEnvironment,
        SandboxFactKind, SandboxGuardrailEffect, SandboxLifecycle, SandboxManifest, SandboxPolicy,
        SandboxRequest,
    };
    ...
    fn fact(facts: &[crucible_sandbox::SandboxAuditRecord], index: usize) -> &SandboxFactKind {
        facts
            .get(index)
    ...
    fn a_command_deadline_is_enforced_without_a_bash_waiter() {
        ...
        let audit = SandboxAudit::new(ancestry, call.clone());
        let limits = crucible_sandbox::SandboxResourceLimits {
        ...
        assert_eq!(
            violation,
            Some(crucible_sandbox::SandboxViolation::CommandTime)
        );
        let facts = audit.records().expect("facts");
        let kinds = facts
            .iter()
        ...
        let violated = kinds
            ...
            .position(|kind| {
                matches!(
                    kind,
                    SandboxFactKind::Violation(crucible_sandbox::SandboxViolation::CommandTime)
                )
            })
    ...
    fn process_streams_enforce_the_combined_output_ceiling() {
        ...
        loop {
            ...
            match output.read_ready(&mut buffer).expect("read") {
                crucible_sandbox::SandboxRead::Bytes(read)
                | crucible_sandbox::SandboxRead::Limited { retained: read, .. } => {
        ...
        while stdout.is_some() || stderr.is_some() {
            for stream in [&mut stdout, &mut stderr] {
                ...
                match output.read_ready(&mut buffer).expect("read") {
                    ...
                    crucible_sandbox::SandboxRead::Pending => {}
                    crucible_sandbox::SandboxRead::End => *stream = None,
                }
            }
    ...
    fn drained_under(name: &str, ceiling: Option<u64>) -> (usize, crucible_sandbox::SandboxUsage) {
        ...
        while finished.iter().any(|done| !done) {
            for (stream, done) in streams.iter_mut().zip(finished.iter_mut()) {
                ...
                match output.read_ready(&mut buffer).expect("read") {
                    ...
                    crucible_sandbox::SandboxRead::Pending => {}
                    crucible_sandbox::SandboxRead::End => *done = true,
                }
            }
            assert!(Instant::now() < deadline, "streams did not finish");
            thread::sleep(Duration::from_millis(5));
        ...
        for stream in &mut streams {
            ...
            assert!(matches!(
                ...
                crucible_sandbox::SandboxRead::End
            ));
        }
        let usage = process.usage();
        assert_eq!(
