...
fn compatibility_matrix_is_complete_and_never_claims_kernel_confinement() {
    let (identity, capabilities) = crate::local::compatibility_capabilities().expect("identity");
    assert_eq!(
        identity.provenance(),
...
fn linux_matrix_is_complete_and_claims_only_implemented_boundaries() {
    let capabilities = crate::linux::declared_capabilities();
...
fn macos_matrix_is_complete_and_claims_only_implemented_boundaries() {
    assert_exact_matrix(
        &crate::macos::declared_capabilities(),
        MACOS_ENFORCED,
        OBSERVED,
    );
}
...
fn windows_matrix_is_complete_and_claims_only_implemented_boundaries() {
    assert_exact_matrix(
        &crate::windows::declared_capabilities(),
        WINDOWS_ENFORCED,
        OBSERVED,
    );
}
