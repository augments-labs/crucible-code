...
pub(super) fn prepare(request: &SandboxRequest) -> Result<View, SandboxError> {
    ...
    if limits.processes.is_some()
        && super::probe::process_limit() != crucible_sandbox::SandboxCapability::Enforced
    {
...
fn append_resource_limits(process: &mut Command, limits: crucible_sandbox::SandboxResourceLimits) {
    for (name, value) in [
...
mod tests {
    ...
    use crucible_sandbox::{SandboxManifest, SandboxPolicy, SandboxRequest};
    ...
    fn a_confined_command_carries_the_standard_ceilings_to_the_broker() {
        ...
        let read_only = SandboxPolicy::new(
            ...
            sample.root().clone(),
            SandboxNetworkPolicy::Closed,
            crucible_sandbox::SandboxResourceLimits {
                ...
                processes: Some(64),
                ..crucible_sandbox::SandboxResourceLimits::confining()
            },
        )
        ...
        let command = SandboxCommand::new(
            ...
            crucible_sandbox::SandboxEnvironment::empty(),
        )
    ...
    fn protected_symlinks_and_unreadable_missing_entries_fail_deterministically() {
        ...
        let linked = SandboxFilesystemRule::new(
            sample.root().join("linked"),
            SandboxFilesystemAccess::Protected,
            crucible_sandbox::SandboxFilesystemProvenance::ProtectedMetadata,
        )
        .expect("linked rule");
        ...
        let missing = SandboxFilesystemRule::new(
            ...
            SandboxFilesystemAccess::Unreadable,
            crucible_sandbox::SandboxFilesystemProvenance::Descendant,
        )
        .expect("missing rule");
