...
fn domain_policy() -> crucible_sandbox::SandboxDomainPolicy {
    crucible_sandbox::SandboxDomainPolicy::new(
        [crucible_sandbox::SandboxDomainPattern::new("127.0.0.1").expect("literal")],
        [],
        false,
        [],
        crucible_sandbox::SandboxNetworkProvenance::User,
    )
...
fn startup_unconfirmed_cleanup_retains_linux_projection_and_audits_failed() -> io::Result<()> {
    ...
    let request = SandboxRequest::new(
        ...
        crucible_types::ToolId::new("startup-cleanup"),
        policy,
...
fn pretransfer_network_cleanup_failure_is_quarantined_and_never_complete() -> io::Result<()> {
    ...
    let mut plan =
        testing_plan(crucible_sandbox::SandboxSpeech::Closed, None).map_err(io::Error::other)?;
    let rule = crucible_sandbox::SandboxFilesystemRule::new(
        sample.root(),
        SandboxFilesystemAccess::ReadOnly,
        crucible_sandbox::SandboxFilesystemProvenance::Workspace,
    ...
    let policy = crucible_sandbox::SandboxPolicy::new(
        ...
        [rule],
        sample.root(),
        crucible_sandbox::SandboxNetworkPolicy::Domains(domain_policy()),
        crucible_sandbox::SandboxResourceLimits::default(),
    )
    .map_err(io::Error::other)?;
    let request = SandboxRequest::new(
        plan.sandbox,
        crucible_types::Ancestry::new(),
        crucible_types::ToolId::new("startup-network-cleanup"),
        policy,
        crucible_sandbox::SandboxManifest::empty(),
    );
    plan.audit = request.audit().clone();
...
fn pretransfer_materialization_cleanup_failure_retains_projection_evidence() -> io::Result<()> {
    ...
    .map_err(io::Error::other)?;
    let manifest =
        crucible_sandbox::SandboxManifest::new([crucible_sandbox::SandboxManifestEntry::file(
            "input.txt",
            Box::<[u8]>::from(&b"input\n"[..]),
    ...
    let request = SandboxRequest::new(
        plan.sandbox,
        crucible_types::Ancestry::new(),
        crucible_types::ToolId::new("startup-materialization-cleanup"),
        policy,
        manifest,
    );
    plan.audit = request.audit().clone();
