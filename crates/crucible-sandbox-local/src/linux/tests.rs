...
use crucible_sandbox::{
    SandboxCleanup, SandboxCommand, SandboxCredentialHandle, SandboxCredentialProjection,
    SandboxCredentialProvenance, SandboxEnvironment, SandboxFactKind, SandboxFilesystemAccess,
    SandboxFilesystemProvenance, SandboxFilesystemRule, SandboxInvocationMode, SandboxLifecycle,
    SandboxManifest, SandboxManifestEntry, SandboxNetworkPolicy, SandboxOutput, SandboxPolicy,
...
pub(super) fn request(sample: &Sample, manifest: SandboxManifest) -> SandboxRequest {
    SandboxRequest::new(
        ...
        Ancestry::new(),
        ToolId::new("manifest"),
        crucible_sandbox::SandboxPolicy::standard(&sample.workspace()).expect("policy"),
        manifest,
    )
}
...
/// The durable key a runner would have derived for `call`, so a background
/// command's result has somewhere to be accepted.
fn durable_result_key(call: &str) -> CallResultKey {
...
fn lifecycles(audit: &crucible_sandbox::SandboxAudit) -> Vec<SandboxLifecycle> {
    audit
        .records()
...
fn background_ownership_precedes_release_and_command_start() {
    ...
    let key = durable_result_key("manifest");
    let request = request(&sample, SandboxManifest::empty())
...
fn read_only_background_commands_have_a_durable_lifecycle() {
    ...
    let key = durable_result_key("read-only-background");
...
fn sandbox_crash_helper_process() {
    ...
    let workspace = crucible_workspace::Workspace::open(root).expect("helper workspace");
...
fn abrupt_host_loss_kills_the_scope_and_the_next_prepare_reconciles_its_wal() {
    ...
    let mut helper = Command::new(executable)
        .args([
            "--exact",
            "linux::tests::sandbox_crash_helper_process",
            "--nocapture",
            "--test-threads=1",
        ])
