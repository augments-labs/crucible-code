...
fn full_audit(plan: &SpawnPlan) -> io::Result<()> {
    for _ in 0..crucible_sandbox::MAX_SANDBOX_AUDIT_FACTS {
        plan.audit
            .record(
                plan.sandbox,
...
fn startup_stop_failure_is_bounded_and_quarantines_owned_resources() -> io::Result<()> {
    ...
    let plan = testing_plan(
        crucible_sandbox::SandboxSpeech::Held,
    ...
    assert_eq!(
        (
            bounded,
            matches!(result, Err(crucible_sandbox::SandboxError::Lifecycle(_))),
...
fn startup_audit_failure_with_proved_cleanup_releases_resources() -> io::Result<()> {
    ...
    assert!(matches!(
        result,
        Err(crucible_sandbox::SandboxError::Audit(_))
    ));
    assert!(!root.exists());
