...
fn the_status_row_is_the_mode_in_its_own_colour_and_everything_else_quietly() {
    ...
    // what a tool call arriving now costs. Everything after it is quiet, so
    // nothing competes with it for the eye.
    pictured("status", &leaving("", 2), 80, Glyphs::Unicode);
}
...
fn what_is_still_running_is_named_on_the_status_row_and_nowhere_else() {
    ...
    assert_eq!(
        rows.len(),
        typed("").rows(80, Glyphs::Unicode).len(),
...
fn nothing_running_makes_that_row_no_door() {
    // With nothing to name, the row is the mode and the keys — facts rather than
