...
//!
//! **No background is ever written.** An inline renderer draws into the
//! terminal's own buffer, so the ground behind every row is the one the reader
//! chose, and it stays that way by emitting no background attribute at all —
//! not by detecting theirs. That is also why one palette serves every terminal:
//! asking would mean a blocking round-trip on a startup path budgeted at 20 ms,
//! for one hue, so instead every colour here clears 3:1 against a black ground
//! *and* a white one, and the test at the bottom is where that is checked.
//!
