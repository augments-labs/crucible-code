...
#[test]
fn what_a_command_may_reach_is_wider_than_what_a_file_tool_may_write() {
    // One list, two questions, and they are not the same width on purpose. A
    // file tool names the file it will write, so the refusal is exactly the
    ...
    // A command names a word and lets the program work the rest out, so the
    // question a proof is answered against is the directory and everything
    // under it. Losing a proof costs a question and nothing more, which is what
    // makes the wider answer the affordable one here.
    assert!(reaches_configuration(Path::new("/w/.crucible/config.json")));
    assert!(reaches_configuration(Path::new("/w/.crucible/notes.json")));
    assert!(reaches_configuration(Path::new("/w/.crucible")));
    assert!(reaches_configuration(Path::new(
        "/w/tools/agent/.crucible/skills/one.md"
    )));

    // Neither of them reaches a directory that merely ends in the name.
    assert!(!reaches_configuration(Path::new(
        "/w/x.crucible/config.json"
    )));
}
