//! Whether a hosted program that ended may be started again.
//!
//! Dying once is not a reason to give up on a program. A process that fell over
//! on its first frame is often perfectly well on its second, and a program that
//! is unavailable for the rest of a run because of one bad start is a worse
//! outcome than trying again. Dying repeatedly is a different thing: at some
//! point the restarts are the failure rather than the recovery, and something
//! has to be counting.
//!
//! Counting is only half of it. The other half is that a program can die at a
//! moment when starting it again would be unsafe rather than merely futile. If
//! crucible was waiting on a call when the process went away, nothing on this
//! side can see how far it got with it — asked for and not begun, begun and not
//! finished, or finished with the answer lost on the way back are the same
//! silence from here. Asking again would be asking for whatever it was a second
//! time, and this cannot know whether that is harmless. So an ending like that
//! ends the program for the run, however much of the ceiling is left.
//!
...
/// Whether what an ended program was doing is known to have finished.
///
/// The usual source is whatever the host was still waiting on when the process
/// went away: those calls are exactly the ones whose effect cannot be seen from
/// here. A host that knows its own outstanding calls were harmless to repeat
/// may say so; nothing in this module can know that for it.
...
/// How many more times one program may be started again, and how many it has
/// already spent.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Restarts {
    /// The most restarts this program is allowed in one run.
    ceiling: u32,
...
impl Restarts {
    ...
    ///
    /// A ceiling of zero is the honest way to say a program gets one start and
    /// no more, rather than a policy that has to be spelled somewhere else.
    #[must_use]
...
/// Permission to start one program again, which only [`Restarts::again`]
/// gives.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl Restarting {
    /// Which restart this is, counting from one.
    ///
    /// The number a diagnostic needs to say whether a program is recovering or
    /// circling.
    #[must_use]
