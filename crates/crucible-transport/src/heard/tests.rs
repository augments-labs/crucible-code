...
fn hearing(steps: impl IntoIterator<Item = Step>) -> Frames<Heard<Says>> {
    Frames::new(Heard::new(Says::new(steps), PATIENCE))
}

/// The same, waiting a chosen pause between polls.
fn patiently(steps: impl IntoIterator<Item = Step>, pause: Duration) -> Frames<Heard<Says>> {
    Frames::new(Heard::with_pause(Says::new(steps), PATIENCE, pause))
}
...
fn patience_is_for_one_silence_and_not_for_the_conversation() {
    ...
    let mut frames = patiently(steps, PAUSE);
...
fn a_program_that_says_nothing_for_long_enough_is_given_up_on() {
    let mut frames = patiently([Step::Waits], PAUSE);
