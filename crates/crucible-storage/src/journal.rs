...
//! recovery and extension state, and that whole history belongs here rather
//! than in the runtime that fills it: the identity a stored call result is
//! keyed by, the receipt a durable sink returns, the records a run retains, and
//! the redacted sandbox facts among them. A store is written against this
//! module and links no sandbox, no model and no provider to do it.
//!
//! What a record *means* and what *produced* it stay apart. Everything here
...
/// keeps. A journal cache fact and a cache checkpoint hold the same words in the
/// same build, so they are bounded by the same figure.
pub use crucible_types::MAX_CHECKPOINT_WORD_BYTES as MAX_JOURNAL_WORD_BYTES;
...
impl JournalError {
    ...
    /// crate, so the refusal is reported in this crate's error — but the rule is
    /// not restated here. It is owned once by `crucible-types`, which every
    /// crate retaining one of the words this rule bounds already names, so the
    /// cache words a journal line carries are admitted and refused exactly as a
    /// cache checkpoint admits and refuses them.
    ///
    /// # Errors
    ///
