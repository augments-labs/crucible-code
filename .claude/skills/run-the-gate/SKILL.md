...
One command, every section, no arguments. CI runs this exact script, so a green
...
| `agent rules files` | `AGENTS.md` stopped being a symlink to `CLAUDE.md`, or `.agents/rules` stopped being one to `../.claude/rules`. Restore it; never let two copies exist. |
| `agent rules scope` | A file in `.claude/rules/` has no `paths:` frontmatter, or aims at a directory that no longer exists — either way nothing loads it. Or a package has no rule aimed at it: every crate, and `src/`, needs one. A rule with no `paths:` at all is legal: that is how the harness spells "load this always". |
