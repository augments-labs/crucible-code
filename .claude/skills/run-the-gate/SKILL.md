...
| `agent rules scope` | A file in `.agents/rules/` has no `paths:` frontmatter, or aims at a directory that no longer exists — either way nothing loads it. Or a package has no rule aimed at it: every crate, and `src/`, needs one. |
