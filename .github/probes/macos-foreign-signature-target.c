/* Controlled root bystander: fixed status protocol, no arbitrary execution. */
#include <errno.h>
#include <mach/mach.h>
#include <mach/ndr.h>
...
int main(void) {
    ...
    if(setrlimit(RLIMIT_CORE,&core)||setrlimit(RLIMIT_FSIZE,&files)||getuid()!=0||geteuid()!=0) return 77;
    ...
    if(getchar()!='f'||state("foreign")) return 77;
