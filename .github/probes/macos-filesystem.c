...
static int operation(const char *name, const char *root, const char *outside) {
    ...
    if(!strcmp(name,"protected-clear-rename") || !strcmp(name,"protected-chmod-rename")) {
        path(a,root,"rw/nested/.git"); path(b,root,"rw/nested/.GIT");
        errno=0; int changed=!strcmp(name,"protected-clear-rename")?chflags(a,0):chmod(a,0777); int why=errno;
        printf("PROTECTION-CHANGE name=%s result=%d errno=%d\n",name,changed,why);
        errno=0; return rename(a,b);
    }
    if(!strcmp(name,"protected-fchmod-write")) {
        path(a,root,"rw/nested/.git/config"); int fd=open(a,O_RDONLY); if(fd<0) return -1;
        errno=0; int changed=fchmod(fd,0777); int why=errno; close(fd);
        printf("PROTECTION-CHANGE name=%s result=%d errno=%d\n",name,changed,why);
        errno=0; return open(a,O_WRONLY);
    }
