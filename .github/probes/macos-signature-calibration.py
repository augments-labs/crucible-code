"""One disposable Intel fixture; add only its unique detached signature."""
import hashlib
import os
import pathlib
import re
...
require(str(root).startswith('/private/tmp/crucible-macos-signature.')
        and root.resolve(strict=True) == root and stat.S_ISDIR(root.lstat().st_mode), 'fixture root')
os.chown(root, 0, 0)
os.chmod(root, 0o755)
for name in ('target', 'nonce.h', 'macos-signature-calibration.c', 'macos-signature-calibration.py'):
    path = root / name
    info = path.lstat()
    require(stat.S_ISREG(info.st_mode) and info.st_nlink == 1 and info.st_size <= 1024*1024, 'fixture file')
    os.chown(path, 0, 0)
    os.chmod(path, 0o755 if name == 'target' else 0o644)
binary = root / 'target'
...
identifier = 'com.crucible.disposable-signature.' + nonce
...
target = subprocess.Popen([str(binary)], stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                          stderr=subprocess.STDOUT, env={'PATH':'/usr/bin:/bin'}, preexec_fn=drop)
pending = bytearray()
output_bytes = 0
...
try:
    require(line() == 'READY nonce='+nonce+' uid='+str(uid)+' pid='+str(target.pid), 'direct owned target identity')
    before = state('before')
    ...
    print('CALIBRATION effect_observed=1 self_only=1 full_backend=0', flush=True)
finally:
    cleanup(target, 'target')
    print('CALIBRATION-CLEANUP direct_child_reaped=1 fixture_and_own_database_record_retained_for_VM_disposal=1', flush=True)
