...
Twelve tools, and the model chooses which one to call. Three of them only look —
...
something to answer them. Four change nothing outside crucible: `bash_output`
says what a command left running has printed so far; `todo_write` puts down the
plan, and you read it above the prompt; `ask_user` puts a question to you and
waits; and `tool_search` finds the tools that are not in the list —
because some are held back until they are asked for, which keeps a session from
paying for schemas it never uses.
